...
function gcbActivityAudit(data_dict) {
  console.log("RAM: activity-generic gcbActivityAudit()");
  if (gcbCanPostEvents) {
...
function renderActivity(contentsLst, domRoot) {
  console.log("RAM: activity-generic renderActivity()");
...
function renderAssessment(assessment, domRoot) {
  ...
  $.each(assessment.questionsList, function(questionNum, q) {
    ...
    // ------------ CUSTOMIZATION -------------------
    // If this is a Quizme question type
    // Create an iframe and load it from blockly.html
    // Quizme assessments handle their own evaluations -- see README.QUIZME
...
// --------------- CUSTOMIZATION ----------------------------
// Quizme assessments have their own buttons
...
//        disabledHtml + '>' + buttonText + '</button>');
  ...
  // Quizme modification
  if (assessment.checkAnswers && !assessment.hasCheckAnswerBtn) {
  ...
  if (!assessment.hasSubmitBtn)
    domRoot.append('<br/><a class="gcb-button gcb-button-primary" id="submitAnswersBtn">Submit</a>'); 

  $('#checkAnswersBtn').click(function() {checkOrSubmitAnswers(assessment, false);});
  $('#submitAnswersBtn').click(function() {checkOrSubmitAnswers(assessment, true);});
 ...
 * 
 *  NOTE: Function createBlocklyFrame() and other functions for processing assessments
 *   have been moved to 
 *         assets/lib/quizly/activity-quizme.js
 *   which must be inmported as a script in views/base-course.html
 *  
 */

  function checkOrSubmitAnswers(submitAnswers) {
    console.log("RAM: activity-generic checkOrSubmitAnswers()");
    $('#answerOutput').html('');
    ...
    $.each(assessment.questionsList, function(questionNum, q) {
      ...
      // -------------- CUSTOMIZATION ----------------------
      // Quizme:  Processing the results of a Quizme assessment
      else if (q.correctAnswerQuizme) {
